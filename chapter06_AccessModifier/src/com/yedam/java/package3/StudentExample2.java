...
public class StudentExample2 {
	public static void main(String[] args) {
		...
		Student[] scoreList = null; // 학생들의 점수목록
		...
		while (run) {
			...
			if (selectNo == 1) { // 학생수 입력 => 배열의 크기
				 ...
				 */
				MaxScore korMax = new MaxScore(0, scoreList[0].getKoScore());
				MaxScore mathMax = new MaxScore(0, scoreList[0].getMaScore());
				MaxScore engMax = new MaxScore(0, scoreList[0].getEnScore());
				
				for(int i=0; i < studentNum; i++) {
					Student student = scoreList[i];
					if(korMax.getScore() < student.getKoScore()) {
						korMax.setIdx(i);
					}
					if(mathMax.getScore() < student.getMaScore()) {
						mathMax.setIdx(i);
					}
					if(engMax.getScore() < student.getEnScore()) {
						engMax.setIdx(i);
					}
				}
				System.out.printf("국어 최고점을 받은 학생은 %s이고 점수는 %d\n", scoreList[korMax.getIdx()].getName(), scoreList[korMax.getIdx()].getKoScore());
				System.out.printf("수학 최고점을 받은 학생은 %s이고 점수는 %d\n", scoreList[mathMax.getIdx()].getName(), scoreList[mathMax.getIdx()].getMaScore());
				System.out.printf("영어 최고점을 받은 학생은 %s이고 점수는 %d\n", scoreList[engMax.getIdx()].getName(), scoreList[engMax.getIdx()].getEnScore());
